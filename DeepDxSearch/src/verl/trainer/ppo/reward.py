...
# def compute_reward(data: DataProto, reward_fn: AbstractRewardManager, **kwargs) -> tuple[torch.Tensor, dict[str, Any]]:
#     """
...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager, **kwargs) -> tuple[torch.Tensor, dict[str, Any]]:
    ...
    Args:
        data: DataProto object containing the input data.
        reward_fn: Reward function to compute the reward.
        **kwargs: Additional arguments, e.g., curr_save_path.
    Returns:
        Tuple of reward tensor and extra info dictionary.
    """
    try:
        if "curr_save_path" in kwargs and reward_fn.is_token_apply:
            reward_result = reward_fn(data, return_dict=True, curr_save_path=kwargs["curr_save_path"])
        else:
            reward_result = reward_fn(data, return_dict=True)
        ...
        print(f"Error in reward_fn: {e}")
        if "curr_save_path" in kwargs and reward_fn.is_token_apply:
            reward_tensor = reward_fn(data, curr_save_path=kwargs["curr_save_path"])
        else:
            reward_tensor = reward_fn(data)
