...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    # begin of modification. Here disable vision parts
    if default_transformer_cls_names_to_wrap is not None:
        vision_modules = {'SiglipVisionEmbeddings', 'SiglipEncoderLayer', 'SiglipMultiheadAttentionPoolingHead'}
        default_transformer_cls_names_to_wrap = [
            cls_name for cls_name in default_transformer_cls_names_to_wrap 
            if cls_name not in vision_modules
        ]
        if not default_transformer_cls_names_to_wrap:
