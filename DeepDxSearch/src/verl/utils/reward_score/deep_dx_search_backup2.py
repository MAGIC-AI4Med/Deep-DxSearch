...
def extract_all_tag_pairs(text: str) -> dict:
    ...
    Returns:
        dict: {
            ...
            'answer': None 或 List[str]
        }
    """
    tag_names = ['think', 'lookup', 'match', 'reflect', 'guide', 'refer', 'answer']
    result = {}
...
def validate_format(text: str) -> tuple[bool, str]:
    """
    验证格式规则：
    1. <answer></answer> pair 至少出现一次，且最后一个</answer>必须是结束前的最后字符
    2. <lookup></lookup> pair 最多出现3次
    3. <match></match> pair 最多出现3次
    4. 最后一个<answer> pair中必须包含 \\textbf{}，且最多5次
    ...
    # 规则1: 检查answer pair
    answer_pairs = all_pairs.get('answer')
    if answer_pairs is None or len(answer_pairs) == 0:
        return False, "<answer></answer> pair must appear at least once"
    
    # 检查最后一个</answer>是否是结束前的最后字符
    last_answer_close_pos = text.rfind('</answer>')
    if last_answer_close_pos == -1:
        return False, "</answer> not found before end"
    
    # 去除尾部空白后检查
    text_stripped = text.rstrip()
    if not text_stripped.endswith('</answer>'):
        return False, "</answer> must be the last content (ignoring trailing whitespace)"
    ...
    # 规则4: 检查最后一个answer pair中的\\textbf{}
    last_answer_content = answer_pairs[-1]
    textbf_matches = re.findall(r'\\textbf\{.*?\}', last_answer_content)
    
    if not textbf_matches:
        return False, "Last <answer> pair must contain at least one \\textbf{}"
    
    if len(textbf_matches) > 5:
        return False, f"\\textbf{{}} appears {len(textbf_matches)} times in last <answer> pair (max 5)"
    
    # 规则5: 检查非think pair中不能包含结束标签
    # 定义所有可能的结束标签
    all_close_tags = ['</think>', '</lookup>', '</match>', '</reflect>', '</guide>', '</refer>', '</answer>']
...
def extract_diseases_from_textbf(text: str) -> tuple:
    ...
    try:
        # 使用extract_tag_pairs提取answer内容
        answer_pairs = extract_tag_pairs(text, 'answer')
        
        if answer_pairs is None:
            return None, "No answer tag found"
        
        # 取最后一个answer的内容
        answer_content = answer_pairs[-1]
