...
def extract_trajectory(text: str) -> Optional[str]:
    """
    按照顺序提取 lookup, match 和 search 标签对的出现轨迹
    
    Args:
        text: 输入文本
    
    Returns:
        Optional[str]: 轨迹字符串，如 "lookup,match,search,lookup,match"，出错则返回 None
    """
    try:
        # 提取所有 lookup, match 和 search pair 及其结束标签位置
        lookup_pairs = extract_tag_pairs_with_positions(text, 'lookup')
        match_pairs = extract_tag_pairs_with_positions(text, 'match')
        search_pairs = extract_tag_pairs_with_positions(text, 'search')
        ...
        if search_pairs is not None:
            for close_pos, content in search_pairs:
                all_tags.append((close_pos, 'search'))
...
def extract_all_tag_pairs(text: str) -> dict:
    ...
    Returns:
        dict: {
            ...
            'search': None 或 List[str],
            'reflect': None 或 List[str],
            'guide': None 或 List[str],
            'refer': None 或 List[str],
            'result': None 或 List[str],
            'diagnose': None 或 List[str]
        }
    """
    tag_names = ['think', 'lookup', 'match', 'search', 'reflect', 'guide', 'refer', 'result', 'diagnose']
    result = {}
...
def validate_format(text: str) -> tuple[bool, str]:
    ...
    4. <search></search> pair 最多出现3次
    5. 最后一个<diagnose> pair中必须包含 \\textbf{}，且最多5次
    6. 除了<think></think>之间的内容，其他tag pair内容中不允许包含任何结束标签
    """
    ...
    # 规则4: 检查search pair数量
    search_pairs = all_pairs.get('search')
    if search_pairs is not None and len(search_pairs) > 3:
        return False, f"<search></search> pair appears {len(search_pairs)} times (max 3)"
    
    # 规则5: 检查最后一个diagnose pair中的\\textbf{}
    ...
    # 规则6: 检查非think pair中不能包含结束标签
    # 定义所有可能的结束标签
    all_close_tags = ['</think>', '</lookup>', '</match>', '</search>', '</reflect>', '</guide>', '</refer>', '</result>', '</diagnose>']
...
def extract_all_search_queries(text: str) -> tuple:
    """
    从所有search标签中提取查询内容列表（每个search一个列表）
    
    Args:
        text: 输入文本
    
    Returns:
        tuple: (all_search_queries, reason)
            - all_search_queries: List[str]，每个元素是一个search标签的完整查询内容
            - reason: 成功或失败原因
    """
    try:
        # 使用extract_tag_pairs提取所有search内容
        search_pairs = extract_tag_pairs(text, 'search')
        ...
        # 提取每个search中的查询内容
        all_search_queries = []
        for search_content in search_pairs:
            search_content = search_content.strip()
            if not search_content:
                all_search_queries.append("")
                continue
            
            all_search_queries.append(search_content)
        
        return all_search_queries, "successful extraction"
...
def check_search_query_disease_match(search_query: str, ground_truth: List[str]) -> bool:
    """
    检查search查询中是否包含ground truth中的任何疾病
    
    Args:
        search_query: search标签的完整查询内容，如 "|WIKI| disease1 |PMC| disease2"
        ground_truth: 真实疾病列表
    
    Returns:
        bool: 如果查询中包含任何ground truth疾病则返回True
    """
    # 标准化查询内容
    normalized_query = enhanced_normalize_answer(search_query)
    
    # 提取查询中的tokens
    query_tokens = set(get_tokens(normalized_query))
    
    # 检查每个ground truth疾病
    for gt_disease in ground_truth:
        normalized_gt = enhanced_normalize_answer(gt_disease)
        gt_tokens = get_tokens(normalized_gt)
        
        # 如果ground truth疾病的所有token都出现在查询中，认为命中
        if all(token in query_tokens for token in gt_tokens):
            return True
...
def compute_score(tokenizer, solution_str, ground_truth, trajectory_stats=None) -> tuple:
    ...
    Returns:
        tuple: (final_score, detailed_reason, refer_adjust, lookup_score, search_score, diagnose_score, trajectory, diversity_coefficient)
    """
    # 处理模型输出格式
    ...
    try:
        ...
        return 0.0, f'bad format: invalid response', None, None, None, None, None, 0.0
    ...
    if not response_stripped.endswith('</diagnose>'):
        return 0.0, f'over length or not ending with </diagnose>', None, None, None, None, None, 0.0
    ...
    if not valid_template:
        return 0.0, f'bad format: {reason}', None, None, None, None, trajectory, 0.0
    ...
    # 提取所有search结果并计算分数
    all_search_queries, reason = extract_all_search_queries(response)
    if all_search_queries is None:
        search_score = 0.0
        search_reason = f'no search results: {reason}'
    else:
        # 检查是否有任何search查询命中ground truth
        has_hit = False
        hit_details = []
        
        for idx, search_query in enumerate(all_search_queries):
            if not search_query:
                hit_details.append(f"search #{idx+1}: empty query")
                continue
            
            is_hit = check_search_query_disease_match(search_query, ground_truth)
            hit_details.append(f"search #{idx+1}: {'Hit' if is_hit else 'Miss'}")
            if is_hit:
                has_hit = True
        
        if has_hit:
            search_score = 0.3  # 如果有命中，给0.3分
            search_reason = f'search score: {search_score:.3f} (disease hit). Details: {"; ".join(hit_details)}'
        else:
            search_score = 0.0
            search_reason = f'search score: {search_score:.3f} (no disease hit). Details: {"; ".join(hit_details)}'
    ...
    # 计算最终分数：lookup(5%) + search(5%) + diagnose(90%)
    final_score = lookup_score * 0.1 + search_score * 0.1 + diagnose_score * 0.80
    
    # 确保最终分数不低于0
    final_score = max(0.0, final_score)
    final_score = min(1.0, final_score)
    
    # ============= 计算 diversity coefficient =============
    ...
    if trajectory_stats is not None:
        ...
        if total_count > 320:
            if trajectory in trajectory_stats:
                ...
                # 系数 = sqrt(1 - ratio)
                diversity_coefficient = (1.0 - ratio)
    ...
    detailed_reason = (
        f'Final score: {final_score_with_diversity:.3f} (base: {final_score:.3f}, diversity_coef: {diversity_coefficient:.3f}) | '
        f'Refer adjust: {refer_adjust} ({refer_reason}) | '
        f'Lookup: {lookup_score:.3f} ({lookup_reason}) | '
        f'Search: {search_score:.3f} ({search_reason}) | '
        f'Diagnose: {diagnose_score:.3f} ({diagnose_reason})'
    )
    
    return final_score_with_diversity, detailed_reason, refer_adjust, lookup_score, search_score, diagnose_score, trajectory, diversity_coefficient