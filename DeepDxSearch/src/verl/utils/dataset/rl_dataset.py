...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # modified
        self.apply_chat = self.apply_chat_template_kwargs.get("apply_chat", True)
        self.prompt_template_name = self.apply_chat_template_kwargs.get("prompt_template_name", None)
                ...
                def doc2len(doc) -> int:
                    try:
                        if apply_chat:
                            ...
                            return len(
                                tokenizer.encode(prompt_template.format(prompt=doc[prompt_key]), add_special_tokens=False)
                            )
                    except Exception:
                        print("Error processing one of the samples, skipping...")
                        traceback.print_exc()
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            # For text-only with custom template, just pop the prompt as a string
        ...
        #     attention_mask = model_inputs.pop("attention_mask")
            ...
            if self.apply_chat:
                ...
                # messages here is the original chat content (a string after pop from prompt_key)
                chat = [
                    {'role': 'system', 'content': self.prompt_template},
                    {'role': 'user', 'content': messages}
                ]
                raw_prompt = self.tokenizer.apply_chat_template(
                    chat, add_generation_prompt=True, tokenize=False, **self.apply_chat_template_kwargs
                )
            else:
                raw_prompt = self.prompt_template.format(prompt=messages)
        ...
        if 'reward_model' in row_dict and isinstance(row_dict.get('reward_model', {}).get('ground_truth'), np.ndarray):
