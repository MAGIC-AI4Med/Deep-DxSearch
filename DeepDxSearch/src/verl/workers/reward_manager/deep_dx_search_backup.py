...
class DeepDxSearchRewardManagerWithSave(AbstractRewardManager):
    ...
    def __call__(self, data: DataProto, return_dict: bool = False, curr_save_path=None) -> torch.Tensor | dict[str, Any]:
        ...
        for i in range(len(data)):
            ...
            # 先填入通用的基础信息 (注意加入了你要求的 sequences_str)
            log_entry = {
                'data_source': data_source,
            ...
            # Case 1: 如果是 Tuple (对应你现在的 compute_score 返回 5 个值)
            if isinstance(score_result, tuple):
                # 按顺序解包: final_score, detailed_reason, refer_adjust, lookup_score, answer_score
                final_score, detailed_reason, refer_adjust, lookup_score, answer_score = score_result
                ...
                log_entry['search_score'] = lookup_score
                ...
                reward_extra_info['search_score'].append(lookup_score)
            ...
            if already_print_data_sources[data_source] < self.num_examine:
                ...
                if isinstance(score_result, tuple):
                    ...
                    print(f"[reason] {detailed_reason[:100]}...") # 避免reason太长刷屏
                    print(f"[refer_score] {refer_adjust}")
                    print(f"[search_score] {lookup_score}")
