...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        # )
        # --- 修改开始: 添加对 vllm_with_search 的支持 ---
        if rollout_config.name == 'vllm_with_deep_dx_search':
            # 新增deepdxsearch的支持
            from verl.workers.rollout.vllm_rollout import vLLMRolloutWithDeepDxSearch
            self.rollout = vLLMRolloutWithDeepDxSearch(
                config=rollout_config, model_config=model_config, device_mesh=rollout_device_mesh
            )
        else:
            # 原有逻辑
